...
NTPtime NTPch("ro.pool.ntp.org"); // from this pool we extract information about the current datetime
//we use the pool that holds information about the date and time of Romania.
strDateTime dateTime;//here we'll store the info that we get from the pool
...
void loop() {
  ...
  String hour;
  //Record the time
  // first parameter: Time zone in floating point (for Romania); second: adjustments (0 means no adjustment)
  dateTime = NTPch.getNTPtime(2.0, 0);
  if(dateTime.valid){   //check for valid dateTime
     int actualHour = dateTime.hour;
     int year = dateTime.year;
     int month = dateTime.month;
     int day = dateTime.day;
     if(day < 10 && month < 10)
        date = String ("0"+String(day) + "_0" + String(month) + "_" + String(year));
     else
        if(day<10) date = String ("0"+String(day) + "_" + String(month) + "_" + String(year));
        else if(month < 10) date = String (String(day) + "_0" + String(month) + "_" + String(year));
